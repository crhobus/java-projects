...
public class jpvmEnvironment {
    ...
    public int pvm_spawn(String task_name, int num, jpvmTaskId tids[]) throws jpvmException {
        int ret = 0;
    ...
    public synchronized void pvm_send(jpvmBuffer buf, jpvmTaskId tid, int tag) throws jpvmException {
    ...
    public synchronized void pvm_mcast(jpvmBuffer buf, jpvmTaskId tids[], int ntids, int tag) throws jpvmException {
        int exceptions = 0;
        jpvmMessage message = new jpvmMessage(buf, null, myTid, tag);
        for (int i = 0; i < ntids; i++) {
            jpvmTaskId tid = tids[i];
            message.destTid = tid;
            jpvmDebug.note("pvm_mcast, sending message to " + tid.toString());
            try {
                jpvmSendConnection conn = getConnection(tid);
                message.send(conn);
            } catch (jpvmException jpe) {
    ...
    public jpvmMessage pvm_recv(jpvmTaskId tid, int tag) throws jpvmException {
        //
    ...
    public jpvmMessage pvm_nrecv(jpvmTaskId tid, int tag) throws jpvmException {
    ...
    // Internal methods:
    private void init(boolean isDaemon, String taskName) throws jpvmException {
    ...
    private void findParent() throws jpvmException {
        ...
        if (regStr == null) {
            ...
            try {
                registrationNumber = Integer.valueOf(regStr).intValue();
            } catch (NumberFormatException nfe) {
    ...
    public static String pvm_daemon_file_name() {
        ...
        if (osName.equals("Windows 95") || osName.equals("Windows NT") || true || osName.equals("Windows 3.1")) {
            fileName = "c:\\Windows\\temp\\jpvmd-" + userName + ".txt";
        } else {
