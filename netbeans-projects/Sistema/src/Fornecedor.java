...
public class Fornecedor extends JFrame {
    ...
    private ArrayList<ArrayFornecedor> lista = new ArrayList();
    private ArrayList<ArrayCidade> lista2 = new ArrayList();
    private JTextField tf_Codigo, tf_Nome, tf_Sigla, tf_Comissao, tf_Endereco, tf_Numero, tf_Bairro, tf_Email, tf_Empresa;
    private JComboBox cb_Cidade, cb_Estado;
    private JFormattedTextField CNPJ, IE, CEP, Fone, Celular;
    ...
    Fornecedor() {
        ...
        Painel1.setBounds(10, 10, 418, 268);
        tela.add(Painel1);
        Painel1.setBorder(BorderFactory.createTitledBorder("Fornecedor"));
        ...
        JLabel lb_Sigla = new JLabel("Sigla");
        lb_Sigla.setBounds(20, 78, 80, 14);
        lb_Sigla.setFont(fonte);
        Painel1.add(lb_Sigla);

        tf_Sigla = new JTextField();
        tf_Sigla.setBounds(20, 94, 110, 20);
        Painel1.add(tf_Sigla);
        tf_Sigla.addFocusListener(cores);
        ...
        lb_CNPJ.setBounds(260, 78, 80, 14);
        ...
        lb_CEP.setBounds(140, 150, 80, 14);
    ...
    private void Ok() {
        ...
        if (lista.isEmpty() == true) {
            ...
            for (int i = 0; i < lista.size(); i++) {
                ArrayFornecedor aux = lista.get(i);
            ...
            if (encontrado == true) {
                int resposta = JOptionPane.showConfirmDialog(null, "O fornecedor " + tf_Codigo.getText() + " ja esta inserido na lista deseja Substituilo? ", "Fornecedor", JOptionPane.YES_NO_OPTION);
                if (resposta == 0) {
                    for (int i = 0; i < lista.size(); i++) {
                        ArrayFornecedor aux = lista.get(i);
                        if (INTTexto == aux.getCodigo()) {
    ...
    private void Excluir() {
        ...
        if (lista.isEmpty() == true) {
            ...
            do {
                try {
                    deuErro = false;
                    String StringTexto = JOptionPane.showInputDialog(null, "Informe o Codigo do Fornecedor a ser Excluído:", "Saída de dados", JOptionPane.WARNING_MESSAGE);
            ...
            if (encontrado == true) {
                ...
                JOptionPane.showMessageDialog(null, "Fornecedor não encontrado na lista", "Erro", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private void Recuperar() {
        ...
        if (lista.isEmpty() == true) {
            ...
            do {
                try {
                    deuErro = false;
                    String StringTexto = JOptionPane.showInputDialog(null, "Insira codigo: ", "Fornecedor", JOptionPane.WARNING_MESSAGE);
                    INTTexto = Integer.parseInt(StringTexto);//Converção String para Integer
                } catch (NumberFormatException e) {
                    deuErro = true;
                    JOptionPane.showMessageDialog(null, "Entre com o numero", "Valor Invalido", JOptionPane.ERROR_MESSAGE);
            ...
            for (int i = 0; i < lista.size(); i++) {
                ...
                if (INTTexto == aux.getCodigo()) {//Não importa se texto digitado esta em maiusculo ou minusculo
                    ...
                    String TextoSigla = aux.getSigla();
                    tf_Sigla.setText(TextoSigla);
            ...
            if (encontrado == true) {
                JOptionPane.showMessageDialog(null, "Fornecedor Encontrado e Recuperado da Lista", "Fornecedor", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(null, "Fornecedor não encontrado na lista", "Erro", JOptionPane.ERROR_MESSAGE);
            }
        }
    ...
    private void Ver() {
        if (lista.isEmpty() == true) {
            ...
            VerFornecedor aux = new VerFornecedor(lista);
        }
    }
    ...
    private void Quem() {
        ...
        if (lista.isEmpty() == true) {
            ...
            String Texto = JOptionPane.showInputDialog(null, "Insira nome: ", "Fornecedor", JOptionPane.WARNING_MESSAGE);
            boolean encontrado = false;
            for (int i = 0; i < lista.size(); i++) {
                ArrayFornecedor aux = lista.get(i);
            ...
            if (encontrado == true) {
                JOptionPane.showMessageDialog(null, "O Fornecedor " + Texto + " esta inserido com o codigo " + Codigo, "Fornecedor", JOptionPane.WARNING_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(null, "O Fornecedor " + Texto + " não esta inserido", "Fornecedor", JOptionPane.WARNING_MESSAGE);
            }
        }
    }

    private void Gravar() {
        String textocod, textonumero, InserNome = "", InserSigla = "", InserComissao = "", InserEndereco = "", InserBairro = "", InserCidade = "", InserEstado = "", InserEmail = "", InserEmpresa = "", InserCnpj = "", InserIe = "", InserCep = "", InserFone = "", InserCelular = "";
        ...
        if (!"".equals(tf_Sigla.getText())) {//Se campo não esta vazio
            InserSigla = tf_Sigla.getText();
        } else {//se estiver vazio
            JOptionPane.showMessageDialog(null, "Campo Sigla esta em branco", "Entrada de Dados", JOptionPane.ERROR_MESSAGE);
            tf_Sigla.setText("");//Limpa o campo
            tf_Sigla.grabFocus();//Focaliza o campo
        }
        ...
        if (!"".equals(tf_Codigo.getText()) & !"".equals(tf_Nome.getText()) & !"".equals(tf_Sigla.getText()) & !"".equals(tf_Comissao.getText()) & !"".equals(tf_Endereco.getText()) & !"".equals(tf_Bairro.getText()) & !"Selecione".equals(cb_Cidade.getSelectedItem()) & !"Selecione".equals(cb_Estado.getSelectedItem()) & !"".equals(tf_Email.getText()) & !"".equals(tf_Empresa.getText()) & !"   .   .   /    -  ".equals(CNPJ.getText()) & !"".equals(tf_Numero.getText()) & !"   .   .   ".equals(IE.getText()) & !"     -   ".equals(CEP.getText()) & !"(  )    -    ".equals(Fone.getText()) & !"(  )    -    ".equals(Celular.getText())) {
            ArrayFornecedor aux = new ArrayFornecedor(InserNome, InserSigla, InserComissao, InserEndereco, InserBairro, InserCidade, InserEstado, InserEmail, InserEmpresa, InserCnpj, InserIe, InserCep, InserFone, InserCelular, Insercodigo, Insernumero);
            lista.add(aux);
            JOptionPane.showMessageDialog(null, "Número de Fornecedores cadastrados: " + lista.size(), "Entrada de Dados", JOptionPane.WARNING_MESSAGE);
            LimparTela();
        } else {
    ...
    private void Arquivo() {
        try {
            FileOutputStream arqDados = new FileOutputStream("Fornecedor.dat");
    ...
    private void LerArquivo() {
        try {
            ...
            lista = (ArrayList<ArrayFornecedor>) entra.readObject();
            entra.close();
            int codigoint = lista.size();
            String codigoString = Integer.toString(codigoint + 1);
            tf_Codigo.setText(codigoString);
    ...
    private class TrataCores implements FocusListener {
        ...
        public void focusGained(FocusEvent evento) {//Ganha Focus
            ...
            if (evento.getSource() == tf_Sigla) {
                tf_Sigla.setBackground(Color.YELLOW);
            }
        ...
        public void focusLost(FocusEvent evento) {//Perde Focus
            ...
            tf_Sigla.setBackground(Color.WHITE);
