...
public class ConsultaEndereco extends Componentes {
    ...
    public ConsultaEndereco(EnderecoControl controleEndereco) {
        this.controleEndereco = controleEndereco;
        this.coluna = -1;
        tableModel = new TableModelEndereco(controleEndereco);
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        tabela.getColumnModel().getColumn(4).setMinWidth(240);
        tabela.getColumnModel().getColumn(5).setMinWidth(182);
        ...
        tabela.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evento) {
                if (evento.getClickCount() == 2) {
                    if (listener != null) {
                        listener.exibeEndereco(controleEndereco.getListaPosicao(tabela.getSelectedRow()));
                        dispose();
                    }
                }
            }
    ...
    public void actionPerformed(ActionEvent evento) {
            ...
            if ("".equals(lbNomeColuna.getText())) {
                ...
                if ("".equals(tfProcura.getText().trim())) {
                    ...
                    try {
                        if (coluna == 0) {
                            try {
                                controleEndereco.getLista(coluna, "", Integer.parseInt(tfProcura.getText()));
                            } catch (NumberFormatException ex) {
                            }
                        } else {
                            controleEndereco.getLista(coluna, tfProcura.getText(), 0);
                        }
        ...
        if (evento.getSource() == btCancelar) {
            ...
            controleEndereco.limparLista();
            try {
                controleEndereco.listarEnderecos();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(null, ex.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
            }
            this.coluna = -1;
