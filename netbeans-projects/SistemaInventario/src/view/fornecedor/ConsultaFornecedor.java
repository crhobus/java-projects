...
public class ConsultaFornecedor extends JDialog implements ActionListener {
    ...
    private TableModelFornecedor tableModel;
    private FornecedorDAO fornecedorDAO;
    private ListenerFornecedor listener;
    private int coluna;

    public ConsultaFornecedor(FornecedorDAO fornecedorDAO) {
        this.fornecedorDAO = fornecedorDAO;
        this.coluna = -1;
        tableModel = new TableModelFornecedor(fornecedorDAO);
        initComponents();
    }
        ...
        tabela.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evento) {
                if (evento.getClickCount() == 2) {
                    if (listener != null) {
                        listener.exibeFornecedor(fornecedorDAO.getFornecedorMapa(tabela.getSelectedRow()));
                        dispose();
                    }
                }
            }
                ...
                if ("".equals(tfProcura.getText().trim())) {
                    ...
                    try {
                        if (coluna == 0 || coluna == 6) {
                            try {
                                fornecedorDAO.listFornecedoresCondicao(coluna, "", Integer.parseInt(tfProcura.getText()));
                            } catch (NumberFormatException ex) {
                            }
                        } else {
                            fornecedorDAO.listFornecedoresCondicao(coluna, tfProcura.getText(), 0);
                        }
