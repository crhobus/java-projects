...
public class ConsultaCidade extends JDialog implements ActionListener {
    ...
    public ConsultaCidade(CidadeDAO cidadeDAO) {
        this.cidadeDAO = cidadeDAO;
        this.coluna = -1;
        tableModel = new TableModelCidade(cidadeDAO);
        initComponents();
    }
        ...
        tabela.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evento) {
                if (evento.getClickCount() == 2) {
                    if (listener != null) {
                        listener.exibeCidade(cidadeDAO.getCidadeMapa(tabela.getSelectedRow()));
                        dispose();
                    }
                }
            }
            ...
            if ("".equals(lbNomeColuna.getText())) {
                ...
                if ("".equals(tfProcura.getText().trim())) {
                    ...
                    try {
                        if (coluna == 0) {
                            try {
                                cidadeDAO.listCidadesCondicao(coluna, "", Integer.parseInt(tfProcura.getText()));
                            } catch (NumberFormatException ex) {
                            }
                        } else {
                            cidadeDAO.listCidadesCondicao(coluna, tfProcura.getText(), 0);
                        }
