...
public class ConsultaCep extends JDialog implements ActionListener {
    ...
    public ConsultaCep(CepDAO cepDAO) {
        this.cepDAO = cepDAO;
        this.coluna = -1;
        tableModel = new TableModelCep(cepDAO);
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        tabela.getColumnModel().getColumn(2).setMinWidth(402);
        tabela.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evento) {
                if (evento.getClickCount() == 2) {
                    if (listener != null) {
                        listener.exibeCep(cepDAO.getCepMapa(tabela.getSelectedRow()));
                        dispose();
                    }
                }
            }
            ...
            if ("".equals(lbNomeColuna.getText())) {
                ...
                if ("".equals(tfProcura.getText().trim())) {
                    ...
                    try {
                        if (coluna == 0) {
                            try {
                                cepDAO.listCepsCondicao(coluna, "", Integer.parseInt(tfProcura.getText()));
                            } catch (NumberFormatException ex) {
                            }
                        } else {
                            cepDAO.listCepsCondicao(coluna, tfProcura.getText(), 0);
                        }
