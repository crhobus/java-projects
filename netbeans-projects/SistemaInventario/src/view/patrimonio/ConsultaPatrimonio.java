...
public class ConsultaPatrimonio extends JDialog implements ActionListener {
	...
	private TableModelPatrimonio tableModel;
	private PatrimonioDAO patrimonioDAO;
	private ListenerPatrimonio listener;
	private int coluna;

	public ConsultaPatrimonio(PatrimonioDAO patrimonioDAO) {
		this.patrimonioDAO = patrimonioDAO;
		this.coluna = -1;
		tableModel = new TableModelPatrimonio(patrimonioDAO);
		initComponents();
	}

	private void initComponents() {
		setTitle("Consulta Patrimonios");
		setLayout(null);
		...
		tabela.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent evento) {
				if (evento.getClickCount() == 2) {
					if (listener != null) {
						listener.exibePatrimonio(patrimonioDAO.getPatrimonioMapa(tabela.getSelectedRow()));
						dispose();
					}
				}
			}
	...
	public void setListener(ListenerPatrimonio listener) {
			...
			if ("".equals(lbNomeColuna.getText())) {
				...
				if ("".equals(tfProcura.getText().trim())) {
					...
					try {
						if (coluna == 0) {
							try {
								patrimonioDAO.listPatrimoniosCondicao(coluna, "", Integer.parseInt(tfProcura.getText()), 0);
							} catch (NumberFormatException ex) {}
						} else {
							if (coluna == 4) {
								try {
									patrimonioDAO.listPatrimoniosCondicao(coluna, "", 0, Double.parseDouble(tfProcura.getText()));
								} catch (NumberFormatException ex) {}
							} else {
								patrimonioDAO.listPatrimoniosCondicao(coluna, tfProcura.getText(), 0, 0);
							}
						}
