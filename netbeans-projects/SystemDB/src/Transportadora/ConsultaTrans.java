...
public class ConsultaTrans extends JDialog implements ActionListener {
    ...
    private TableModelTrans tableModel;
    private TransportadoraControl controle;
    private ListenerTrans listener;
    private int coluna;

    public ConsultaTrans(TransportadoraControl controle) {
        this.controle = controle;
        this.coluna = -1;
        tableModel = new TableModelTrans(controle);
        initComponents();
    }

    private void initComponents() {
        setTitle("Consulta Transportadoras");
        setLayout(null);
        ...
        tabela.getColumnModel().getColumn(5).setMinWidth(280);
        ...
        tabela.getColumnModel().getColumn(11).setMinWidth(300);
        tabela.getColumnModel().getColumn(12).setMinWidth(110);
        tabela.getColumnModel().getColumn(13).setMinWidth(100);
        ...
        tabela.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evento) {
                if (evento.getClickCount() == 2) {
                    if (listener != null) {
                        listener.exibeTrans(controle.getListaPosicao(tabela.getSelectedRow()));
                        dispose();
                    }
                }
            }
    ...
    public void setListener(ListenerTrans listener) {
                ...
                if ("".equals(tfProcura.getText().trim())) {
                    ...
                    try {
                        if (coluna == 0 || coluna == 7) {
                            try {
                                controle.getLista(coluna, "", Integer.parseInt(tfProcura.getText()), 0);
                            } catch (NumberFormatException ex) {
                            }
                        } else {
                            if (coluna == 13) {
                                try {
                                    controle.getLista(coluna, "", 0, Double.parseDouble(tfProcura.getText()));
                                } catch (NumberFormatException ex) {
                                }
                            } else {
                                controle.getLista(coluna, tfProcura.getText(), 0, 0);
                            }
                        }
