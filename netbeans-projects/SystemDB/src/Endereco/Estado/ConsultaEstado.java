...
public class ConsultaEstado extends JDialog implements ActionListener {
    ...
    private ListenerEstado listener;
    private int coluna;

    public ConsultaEstado(EstadoControl controleEstado) {
        this.controleEstado = controleEstado;
        this.coluna = -1;
        tableModel = new TableModelEstado(controleEstado);
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        tabela.getColumnModel().getColumn(2).setMinWidth(142);
        tabela.getColumnModel().getColumn(3).setMinWidth(190);
        tabela.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evento) {
                if (evento.getClickCount() == 2) {
                    if (listener != null) {
                        listener.exibeEstado(controleEstado.getListaPosicao(tabela.getSelectedRow()));
                        dispose();
                    }
                }
            }
    ...
    public void setListener(ListenerEstado listener) {
    ...
    public void actionPerformed(ActionEvent evento) {
            ...
            if ("".equals(lbNomeColuna.getText())) {
                ...
                if ("".equals(tfProcura.getText().trim())) {
                    ...
                    try {
                        if (coluna == 0) {
                            try {
                                controleEstado.getLista(coluna, "", Integer.parseInt(tfProcura.getText()));
                            } catch (NumberFormatException ex) {
                            }
                        } else {
                            controleEstado.getLista(coluna, tfProcura.getText(), 0);
                        }
        ...
        if (evento.getSource() == btCancelar) {
            ...
            controleEstado.limparLista();
            try {
                controleEstado.listarEstados();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(null, ex.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
            }
            this.coluna = -1;
