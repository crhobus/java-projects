...
public class ConsultaBairro extends JDialog implements ActionListener {
    ...
    private TableModelBairro tableModel;
    private BairroControl controleBairro;
    private ListenerBairro listener;
    private int coluna;

    public ConsultaBairro(BairroControl controleBairro) {
        this.controleBairro = controleBairro;
        this.coluna = -1;
        tableModel = new TableModelBairro(controleBairro);
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        tabela.getColumnModel().getColumn(3).setMinWidth(230);
        tabela.getColumnModel().getColumn(4).setMinWidth(190);
        ...
        tabela.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evento) {
                if (evento.getClickCount() == 2) {
                    if (listener != null) {
                        listener.exibeBairro(controleBairro.getListaPosicao(tabela.getSelectedRow()));
                        dispose();
                    }
                }
            }
    ...
    public void setListener(ListenerBairro listener) {
    ...
    public void actionPerformed(ActionEvent evento) {
            ...
            if ("".equals(lbNomeColuna.getText())) {
                ...
                if ("".equals(tfProcura.getText().trim())) {
                    ...
                    try {
                        if (coluna == 0) {
                            try {
                                controleBairro.getLista(coluna, "", Integer.parseInt(tfProcura.getText()));
                            } catch (NumberFormatException ex) {
                            }
                        } else {
                            controleBairro.getLista(coluna, tfProcura.getText(), 0);
                        }
        ...
        if (evento.getSource() == btCancelar) {
            ...
            controleBairro.limparLista();
            try {
                controleBairro.listarBairros();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(null, ex.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
            }
            this.coluna = -1;
