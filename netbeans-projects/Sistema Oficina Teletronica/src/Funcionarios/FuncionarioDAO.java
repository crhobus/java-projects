...
public class FuncionarioDAO {
    ...
    public boolean insertFuncionario(Funcionario funcionario) throws Exception {
        try {
            if (con.prepareStatement("SELECT * FROM TBFUNCIONARIO WHERE CODIGO = " + funcionario.getCodigo()).executeQuery().next()) {
                return updateFuncionario(funcionario);
            }
            PreparedStatement stm = con.prepareStatement("INSERT INTO TBFUNCIONARIO (CODIGO, DIGITOCONTA, CRACHA, "
                    + "DESCONTOINSS, NUMERO, IDADE, CONTA, CONTAFGTS, ZONA, SECAO, NOME, RG, CPF, SEXO, ESTADOCIVIL, "
                    + "TIPOPESSOA, ENDERECO, BAIRRO, COMPLEMENTO, REFERENCIA, CIDADE, UF, EMAIL, FONE, CELULAR1, CELULAR2, "
                    + "DESCRICAO, EMPRESA, FONEEMPRESA, FAX, MSN, SKYPE, NATURALIDADE, BANCO, AGENCIA, CARTEIRAHABILITACAO, "
                    + "CARTEIRATRABALHO, CERTIFICADORESERVISTA, TITULOELEITOR, NOMEPAI, NOMEMAE, PAIS, CHEFE, CARGO, "
                    + "SETOR, CEP, RACACOR, HORASSEMANAIS, HORASMENSAIS, HORAINICIAL, HORAFINAL, IE, CNPJ, "
                    + "OUTRASCONSIDERACOES, TIPOCONTRATO, NUMEROCONTRATO, INSCRICAOINSS, PIS, ISENTOIR, CONTRIBUICAOSINDICAL, "
                    + "DATANASCIMENTO, DATACADASTRO, ULTALTERACAO, DATACONTRDEMI, DATAPAGAMENTO, SALARIOINICIAL, AUMENTO, "
                    + "SALARIOLIQUIDO, VALEALIMENTACAO, VALETRANSPORTE, PLANOSAUDE, SALDOFGTS, SEGUROVIDA, ATIVO) VALUES (?, "
                    + "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
                    + "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            throw new Exception("Erro na gravação do funcionário no banco de dados");
        }
    }

    public boolean updateFuncionario(Funcionario funcionario) throws Exception {
        try {
            PreparedStatement stm = con.prepareStatement("UPDATE TBFUNCIONARIO SET DIGITOCONTA = ?, CRACHA = ?, DESCONTOINSS = ?, NUMERO = ?, "
            ...
            throw new Exception("Erro ao regravar o funcionário no banco de dados");
        }
    }

    public boolean deleteFuncionario(String cpf) throws Exception {
        try {
            return con.prepareStatement("DELETE FROM TBFUNCIONARIO WHERE CPF = " + "'" + cpf + "'").executeUpdate() == 1;
        } catch (Exception ex) {
            throw new Exception("Erro ao excluir um funcionário do banco de dados");
        }
    }
    ...
    public Funcionario selectFuncionario(String cpf) throws Exception {
        try {
            ...
            throw new Exception("Erro na leitura do funcionário do banco de dados");
        }
    }
    ...
    public List<Funcionario> listFuncionarios() throws Exception {
        try {
            ...
            ResultSet rs = con.prepareStatement("SELECT * FROM TBFUNCIONARIO ORDER BY CODIGO").executeQuery();
            while (rs.next()) {
                lista.add(getFuncionario(rs));
            }
            return lista;
        } catch (Exception ex) {
            throw new Exception("Erro na leitura de funcinários do banco de dados");
        }
    }
    ...
    public boolean isFuncVazio() throws Exception {
        try {
            return !con.prepareStatement("SELECT * FROM TBFUNCIONARIO").executeQuery().next();
        } catch (Exception ex) {
    ...
    public int getProxCodFunc() throws Exception {
        try {
            ...
            throw new Exception("Erro na leitura de registros de funcionarios do banco de dados");
        }
    }

    public boolean isFuncCadas(String nome) throws Exception {
        try {
            return con.prepareStatement("SELECT * FROM TBFUNCIONARIO WHERE NOME = '" + nome + "'").executeQuery().next();
        } catch (Exception ex) {
            throw new Exception("Erro na leitura do funcionário do banco de dados");
        }
    }
