...
@RestController
@RequestMapping("/api.com/gastoscartaocredito")
public class GastosCartaoCreditoController {

    @Autowired
    private GastosCartaoCreditoService service;

    @Autowired
    private InvokeRestMethod invokeRestMethod;

    @PostMapping(value = "/create", headers = { "X-API-Version=V1" })
    public ResponseEntity<Response<Long>> create(@Valid @RequestBody GastosCartaoCreditoDto dto, BindingResult result) {
    ...
    public ResponseEntity<Response<Boolean>> update(@Valid @RequestBody GastosCartaoCreditoDto dto, BindingResult result) {
    ...
    public ResponseEntity<Response<GastosCartaoCreditoDto>> retrieve(@PathVariable("cpf") String cpf, @PathVariable("id") long id) {
        Response<GastosCartaoCreditoDto> response = new Response<>();

        GastosCartaoCreditoDto dto = service.retrieve(cpf, id);
    ...
    public ResponseEntity<Response<List<GastosCartaoCreditoDto>>> list(@PathVariable("cpf") String cpf) {
        Response<List<GastosCartaoCreditoDto>> response = new Response<>();

        List<GastosCartaoCreditoDto> gastosCartaoCredito = service.list(cpf);
        response.setData(gastosCartaoCredito);
